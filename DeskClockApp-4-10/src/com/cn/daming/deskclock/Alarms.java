...
public class Alarms {
    ...
    private static void clearSnoozeIfNeeded(Context context, long alarmTime) {
        ...
        if (alarmTime < snoozeTime) {//如果闹钟响的时间小于休眠时间清除休眠时间
    ...
    private static long calculateAlarm(Alarm alarm) {
        return calculateAlarm(alarm.month,alarm.day,alarm.hour, alarm.minutes, alarm.daysOfWeek)
                .getTimeInMillis();
    }
     ...
     */
    static Calendar calculateAlarm(int month,int day,int hour, int minute,
    ...
    static String formatYearMonthDay(final Context context, Calendar c) {
     ...
     * settings so those who care can make use of it.3
     * 
     */
